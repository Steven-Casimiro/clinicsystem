...
def searchData(PtntID="", Firstname="", Surname="", DoB="", Age="", Gender="", Address="", Mobile="", Allergies="",CM="",PM="",PS=""):
    con = sqlite3.connect("clinic.db")
    cur = con.cursor()
    cur.execute("SELECT * FROM patient WHERE PtntID=? OR Firstname=? OR Surname=? OR DoB=? OR Age=? OR Gender=? OR Address=? OR Mobile=? OR\
         Allergies=? OR CM=? OR PM=? OR PS=?",(PtntID, Firstname, Surname, DoB, Age, Gender, Address, Mobile, Allergies, CM, PM, PS))
    rows = cur.fetchall()
    con.close()
    return rows
